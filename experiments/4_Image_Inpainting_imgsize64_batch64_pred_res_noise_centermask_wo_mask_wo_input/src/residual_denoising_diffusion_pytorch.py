        ...
        *,
        ...
        if self.condition:
            if len(folder) == 3:
                ...
                # inpaint
                self.condition_type = 3
                ds = dataset("/gpfs/home/liujiawei/data-set/program/diffusion/paper/iclr2024/celeba_hq/val", self.image_size)
                self.sample_dataset = ds
                self.sample_loader = cycle(self.accelerator.prepare(DataLoader(self.sample_dataset, batch_size=num_samples, shuffle=True,pin_memory=True, num_workers=4)))  # cpu_count()
                ds = dataset("/gpfs/home/liujiawei/data-set/program/diffusion/paper/iclr2024/celeba_hq/train", self.image_size)
                self.dl = cycle(self.accelerator.prepare(DataLoader(ds, batch_size=train_batch_size,
                                shuffle=True, pin_memory=True, num_workers=4)))
