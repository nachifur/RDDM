...
class ResidualDiffusion(nn.Module):
    ...
    def init(self):
        ...
        if convert_to_ddim:
            ...
            # adjust
            alphas = gen_coefficients(timesteps, schedule="decreased", ratio=2)
            alphas_cumsum = alphas.cumsum(dim=0).clip(0, 1)
            alphas_cumsum_prev = F.pad(
                alphas_cumsum[:-1], (1, 0), value=alphas_cumsum[1])

            betas2 = gen_coefficients(
                timesteps, schedule="average", sum_scale=self.sum_scale, ratio=1)
            betas2_cumsum = betas2.cumsum(dim=0).clip(0, 1)
            betas2_cumsum_prev = F.pad(
                betas2_cumsum[:-1], (1, 0), value=betas2_cumsum[1])
        else:
            alphas = gen_coefficients(timesteps, schedule="decreased", ratio=1)
            betas2 = gen_coefficients(
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        if self.step != 0 and self.step % (self.save_and_sample_every*10) == 0:
                            ...
                            # os.system("python /home/ljw/data-set/program/diffusion/paper/ablation/celeba/res_diffusion_tempalte_celeba_pred_res_noise_condition_two_op/fid_and_inception_score.py "+gen_img)
                            self.set_results_folder(results_folder)
                if self.num_unet == 1:
